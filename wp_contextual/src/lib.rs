...
//!
//! This presents a challenge in parsing the response because there are several schema variations
...
//! shouldn't have to. The alternative is to have a type for each known variation. The goal
...
//! of [`WpContextual`]. `Sparse` prefix is commonly used to describe types that only have some of
...
//! type back, the proc macro will also generate the `SparseFooWithEditContext`,
//! `SparseFooWithEmbedContext` & `SparseFooWithViewContext` types. Generating these follow the
//! same rules as their non-sparse counterparts, with the main difference being the generated
//! `Sparse` types will not extract `T` from `Option<T>`.
//!
//! ```
//! # use wp_contextual::WpContextual;
//! #[derive(WpContextual)]
...
//! ```
//!
//! This will generate the following sparse types:
//!
//! ```
...
//!     pub baz: Option<u32>,
//! }
//! pub struct SparseFooWithViewContext {
//!     pub baz: Option<u32>,
//! }
//! ```
//!
...
/// `FooWithViewContext` types by turning `Option<T>` fields into `T`; `SparseFooWithEditContext`,
...
/// pub struct SparseFooWithEditContext {
///     pub bar: Option<u32>,
///     pub baz: Option<SparseBazWithEditContext>,
/// }
/// #[derive(Debug, serde::Serialize, serde::Deserialize, uniffi::Record)]
/// pub struct SparseFooWithEmbedContext {
///     pub bar: Option<u32>,
/// }
/// #[derive(Debug, serde::Serialize, serde::Deserialize, uniffi::Record)]
/// pub struct SparseFooWithViewContext {
