...
use std::sync::Arc;
use wp_api::wp_com::{sites::SitesListParams, stats_referrers::StatsReferrersParams};
    ...
    if let Ok(response) = sites_result {
        ...
        for site in &sites {
            ...
            trials.push(Trial::test(
                format!("referrers::get_stats_referrers::{}", site_id),
                {
                    ...
                    move || {
                        ctx.runtime.block_on(async {
                            ctx.client
                                .stats_referrers()
                                .get_stats_referrers(&site_id, &StatsReferrersParams::default())
                                .await
