...
use std::sync::Arc;
use wp_api::wp_com::{sites::SitesListParams, stats_subscribers::StatsSubscribersParams};
    ...
    if let Ok(response) = sites_result {
        ...
        for site in &sites {
            ...
            trials.push(Trial::test(
                format!("subscribers::get_stats_subscribers::{}", site_id),
                {
                    ...
                    move || {
                        ctx.runtime.block_on(async {
                            ctx.client
                                .stats_subscribers()
                                .get_stats_subscribers(&site_id, &StatsSubscribersParams::default())
                                .await
