...
pub fn tests(ctx: Arc<TestContext>) -> Vec<Trial> {
    ...
    if let Ok(response) = sites_result {
        ...
        for site in &sites {
            ...
            trials.push(
                Trial::test(
                    format!("region_views::get_stats_region_views::{site_id}"),
                    {
        ...
        trials.push(Trial::test(
            "region_views::at_least_one_site_authorized".to_string(),
            move || {
                if has_authorized_site {
                    Ok(())
                } else {
                    Err("Expected at least one site with premium access to return region views"
                        .into())
                }
            },
        ));
        ...
        trials.push(Trial::test(
            "region_views::at_least_one_site_unauthorized".to_string(),
            move || {
