...
use std::sync::Arc;
use wp_api::wp_com::{sites::SitesListParams, stats_top_authors::StatsTopAuthorsParams};
    ...
    if let Ok(response) = sites_result {
        ...
        for site in &sites {
            ...
            trials.push(Trial::test(
                format!("top_authors::get_stats_top_authors::{site_id}"),
                {
                    ...
                    move || {
                        ctx.runtime.block_on(async {
                            ctx.client
                                .stats_top_authors()
                                .get_stats_top_authors(&site_id, &StatsTopAuthorsParams::default())
                                .await
