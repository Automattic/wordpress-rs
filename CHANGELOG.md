...
- The `hostname` on `RequestExecutionErrorReason`'s `HttpAuthenticationRequiredError`, `HttpAuthenticationRejectedError` and `HttpForbiddenError` is now the host (with its port, when the URL names one) rather than the entire request URL. These reasons are built for any 401 or 403 whose body isn't a `WpError` — a WAF block, an HTTP Basic auth prompt, a Cloudflare challenge — so the query string went wherever the reason went: into the localized message the reason produces, which names the site to the user, and into any log line that prints the reason. A 403 on `GET /wp/v2/posts/<id>?password=…` put the post's password in both.
