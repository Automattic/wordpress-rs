...
- Kotlin can now read the library's localized, translated message for any error that supports it, via a `localizedDescription(locales)` extension that negotiates an ordered `List<java.util.Locale>` — defaulting to `listOf(Locale.getDefault())`, but accepting a full preferred-language list (e.g. from `LocaleListCompat`) for parity with iOS's fallback chain — e.g. `ParseUrlException.localizedDescription()`, `AutoDiscoveryAttemptFailure.localizedDescription()`. This is the counterpart to Swift's generated `LocalizedError.errorDescription` (which negotiates `Locale.preferredLanguages`); both are generated from the same `WpSupportsLocalization` set and run the same FTL negotiation, so Android no longer hand-rolls English and every localizable type gets its Kotlin message with no per-type edit.
...
- **BREAKING (Kotlin):** `ApiDiscoveryResult` collapsed to a two-case envelope — `Success(AutoDiscoveryAttemptSuccess)` and `Failure(AutoDiscoveryAttemptFailure)` — that passes the values the Rust library emits straight through, mirroring how Swift returns `AutoDiscoveryAttemptSuccess` and throws `AutoDiscoveryAttemptFailure`. The per-variant `FailureParseSiteUrl` / `FailureFindApiRoot` / `FailureFetchAndParseApiRoot` classes, their accessors, and `userFacingErrorMessage()` are gone; match `result.failure` (a sealed `AutoDiscoveryAttemptFailure`) directly and read its localized message with `failure.localizedDescription()`. `WpLoginClient.apiDiscovery()` no longer discards the failure — which is what fixes the misdiagnosis: a private site surfaces the server's own reason (e.g. "This site is private.") instead of "Found a site at $url but failed to read its API configuration.", and `RestApiDisabled` is no longer collapsed to "Could not connect".
- Offline discovery and login failures now render a localized "No internet connection…" message instead of the raw host-resolution error the Kotlin executor previously surfaced. Both platforms share the library's translated `device_is_offline` string; a platform that supplies its own already-localized offline description (e.g. Apple's `URLError`) still has it passed through unchanged.
...
- **BREAKING:** Kotlin: `RequestErrorLogger` is no longer a `fun interface` — it gained a `policy` property — so a logger written as a lambda becomes `WpRequestErrorLogger { … }`. `WpRequestResult.toLogErrorString()` is replaced by `wpApiErrorLogDescription(error, policy)`, which composes the line in Rust so both platforms report a failure the same way.
...
- The `hostname` on `RequestExecutionErrorReason`'s `HttpAuthenticationRequiredError`, `HttpAuthenticationRejectedError` and `HttpForbiddenError` is now the host, with its port when the URL names a non-default one, rather than the entire request URL — so the localized message these produce names the site instead of the whole request, query string and all.
