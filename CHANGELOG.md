...
- **BREAKING:** The `after`, `before`, `modified_after` and `modified_before` list parameters are matched by WordPress against the site's local `post_date` and `post_modified` columns, but were typed `WpGmtDateTime` and sent with a UTC offset — so on a site whose timezone isn't UTC they selected the wrong window. They are now `WpDateString`, which passes the caller's value through unchanged. Callers on a non-UTC site were getting shifted results and will need to supply a site-local datetime. Affects posts, pages, media, comments, blocks, navigation and menu items.
