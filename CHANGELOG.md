...
- **BREAKING:** The `period` on a WordPress.com stats time series was a `String`, and each endpoint that sends one now has its own type: `StatsVisitsPeriod`, `StatsSubscribersPeriod` and `StatsPostViewPeriod`. None of them is a date. The shape follows the unit the caller asked for, and the three endpoints build their labels independently without agreeing — a yearly label is `2013-01-01` on visits and `2013` on subscribers, a weekly one is `2026W02W23`, and the per-post endpoint substitutes a year-less `M-DD` when it has no history to report. Display them or group by them; don't read a date out of one. Each is a record with a single `value` field, so Swift and Kotlin see a distinct type.
- **BREAKING:** Date fields that were `String` are now `WpDateString` — for a value that can't be resolved to an instant, meaning a bare calendar date or a datetime in the site's timezone. This covers `date` and `modified` across posts, pages, media, blocks, navigation, templates, template parts and every revision and autosave resource, `date` on comments, plus the date parameters and response dates on the WordPress.com stats endpoints. It is a record with a single `value` field rather than an alias for a string, so Swift and Kotlin see a distinct type they can hang behaviour on; Rust callers construct it with `WpDateString::new(...)` and read `.value`. `users.registered_date`, application passwords' `created` and `last_used`, and `blog_registration_date` on the WordPress.com individual-subscriber stats are GMT and became `WpGmtDateTime`, which the bindings expose as a date. `CONVENTIONS.md` records the rule.
