...
- **BREAKING:** Date fields that were `String` are now `WpDateString`, so every date in the crate is either that or `WpGmtDateTime` — the former for a value that can't be resolved to an instant (a bare calendar date, or a datetime in the site's timezone), the latter for one that can. This covers `date` and `modified` across posts, pages, media, comments, blocks, navigation, templates, template parts and every revision and autosave resource, plus the date parameters and response dates on the WordPress.com stats endpoints. Callers will need to wrap/unwrap with `WpDateString(...)`; Kotlin and Swift see it as a string, so only the type name changes there. `users.registered_date` and application passwords' `created` and `last_used` are GMT and became `WpGmtDateTime`, which the bindings expose as a date. `CONVENTIONS.md` records the rule and the fields whose names suggest a date but aren't.
- **BREAKING:** Date fields that were `String` are now `WpDateString` — for a value that can't be resolved to an instant, meaning a bare calendar date or a datetime in the site's timezone. This covers `date` and `modified` across posts, pages, media, blocks, navigation, templates, template parts and every revision and autosave resource, `date` on comments, plus the date parameters and response dates on the WordPress.com stats endpoints. Callers will need to wrap/unwrap with `WpDateString(...)`; Kotlin and Swift see it as a string, so only the type name changes there. `users.registered_date` and application passwords' `created` and `last_used` are GMT and became `WpGmtDateTime`, which the bindings expose as a date. `CONVENTIONS.md` records the rule.
- **BREAKING:** Date fields that were `String` are now `WpDateString` — for a value that can't be resolved to an instant, meaning a bare calendar date or a datetime in the site's timezone. This covers `date` and `modified` across posts, pages, media, blocks, navigation, templates, template parts and every revision and autosave resource, `date` on comments, plus the date parameters and response dates on the WordPress.com stats endpoints. Callers will need to wrap/unwrap with `WpDateString(...)`; Kotlin and Swift see it as a string, so only the type name changes there. `users.registered_date`, application passwords' `created` and `last_used`, and `blog_registration_date` on the WordPress.com individual-subscriber stats are GMT and became `WpGmtDateTime`, which the bindings expose as a date. `CONVENTIONS.md` records the rule.
- **BREAKING:** Rust only — every way of reading a `WpGmtDateTime` now shares one parser, so `FromStr`, `Deserialize` and `deserialize_optional_wp_gmt_date_time` accept exactly the same set: an RFC 3339 offset, the offsetless WordPress form (`2026-08-06T09:15:49`, read as UTC), MySQL's (`2026-08-06 09:15:49`), sub-second precision, and a unix timestamp. `FromStr` previously delegated to chrono, which accepted only offset-bearing values but was lenient about their shape; it therefore gains the four offsetless forms and loses an offset written without a colon (`+0000`), which RFC 3339 forbids and neither WordPress core nor WordPress.com was found to emit — core writes REST dates either offsetless via `mysql_to_rfc3339()` or with a colon-separated offset via `gmdate('c')`, and neither form is `+0000`. Kotlin and Swift are unaffected: the bindings lift `WpGmtDateTime` from a unix timestamp and never reach `FromStr`.
- **BREAKING:** Lifting a `WpGmtDateTime` across the bindings now rejects a timestamp that resolves to WordPress's never-set date or to any instant before year 1, instead of returning it as a real value. A timestamp out of range previously became the unix epoch silently; it is now an error too. `WpGmtDateTime::from_timestamp` is no longer public — nothing outside the crate used it, and it survives internally as the unchecked conversion for values that never came from WordPress, such as a certificate's validity bounds.
- **BREAKING:** `users.registered_date`, comments' `date_gmt`, and `blog_registration_date` on the WordPress.com individual-subscriber stats became optional, so Swift and Kotlin see `Date?`. These endpoints format their date column without guarding WordPress's never-set date, so a record that has none sends that value rather than `null` — and reading it as absent keeps one such row from failing the whole response. `CONVENTIONS.md` records which way each endpoint behaves and how to choose.
- **BREAKING:** `Subscriber.date_subscribed` changed from `WpGmtDateTime` to an optional, so Swift and Kotlin see `Date?`. WordPress.com reports a subscription whose date was never set with its never-set date rather than `null`, and that previously failed the whole subscribers response; it now reads as absent.
