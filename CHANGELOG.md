...
- **BREAKING:** `ApiUrlResolver.resolve` now returns `ResolvedUrl` instead of `ParsedUrl`. Custom `ApiUrlResolver` implementations (Swift/Kotlin) must return a `ResolvedUrl` — build one with `ResolvedUrl(url:apiRoot:routePath:)` — and callers that consumed the resolved value as a URL now read `resolve(...).url()` ([#1543](https://github.com/Automattic/wordpress-rs/issues/1543)).
...
- **Internal:** Added a golden URL table (`plain_permalinks_url_tests`) asserting all 45 self-hosted endpoint modules (one representative builder each) resolve to the correct `?rest_route=` URL on a plain-permalinks site, by driving each endpoint's real URL builder through a `rest_route`-seeded `WpOrgSiteApiUrlResolver`. To guard the route paths the table builds on, the 30 non-parameterized routes are also checked (`index_self_href_url_tests`) — on the `wp-json` path form — against WordPress's own published URL, the `_links.self.href` captured in a real-site REST index fixture; the parameterized (ID-bearing) routes, which the index never publishes a URL for, are exercised end-to-end by a new plain-permalinks integration test that fetches a real object by numeric id over `?rest_route=`.
