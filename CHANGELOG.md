...
- **Swift:** `WpRequestExecutor.allowAlternativeNames(_:forCommonName:)` accepts an otherwise-valid certificate whose common name / alternative names don't cover the host being connected to. The chain is still validated — only the hostname check is relaxed. Replaces the ambiguously-named `allowSSL(altNames:forCommonName:)` (now deprecated).
- **Swift:** `WpRequestExecutor.disableCertificateValidation(forHost:)` accepts *any* TLS certificate — self-signed, expired, or issued by an untrusted root — for a named host, disabling validation entirely. It's the explicit, opt-in counterpart to `allowAlternativeNames(_:forCommonName:)`. Intended for hosts the caller controls, such as a local development or staging server; it exposes the connection to man-in-the-middle attacks and should not be used for production hosts.
- **Kotlin:** `WpHttpClient.DefaultHttpClient.disableCertificateValidation(host:)` accepts *any* TLS certificate for a named host by routing it to a separate, validation-disabled OkHttp client. Mirrors Swift's `disableCertificateValidation(forHost:)`; the same man-in-the-middle caveats apply, so use it only for hosts the caller controls.
...
- **Internal:** Point the Swift, Kotlin, and Rust SSL-validation tests — and the login spec's reference implementation — at `badssl.com` instead of bespoke `wpmt.co` / Cloudways hosts, and add a Swift regression test asserting `allowAlternativeNames` still rejects a self-signed certificate. Frees the `vanilla1.wpmt.co` subdomain and the `wordpress-1315525-4803651.cloudwaysapps.com` instance for decommissioning.
...
- **Swift:** `WpRequestExecutor.allowSSL(altNames:forCommonName:)` is deprecated. Its name didn't convey what it did, and it conflated two behaviors that are now separate methods: use `allowAlternativeNames(_:forCommonName:)` to accept an otherwise-valid certificate whose name doesn't cover the host, or `disableCertificateValidation(forHost:)` to accept an invalid or self-signed certificate. The shim forwards to `allowAlternativeNames`, so it now validates the certificate chain (see Security).
...
- **Kotlin:** TLS chain-validation failures (self-signed, expired, or untrusted-root certificates) are now reported as `RequestExecutionErrorReason.InvalidSslError` (with `GenericSslError`) rather than a generic error. Previously only hostname mismatches (`SSLPeerUnverifiedException`) were classified as SSL errors; other `SSLException`s fell through to `GenericError`, losing the "this is a certificate problem" signal.
- The certificate-name exception (`allowAlternativeNames(_:forCommonName:)` on Swift, `addAllowedAlternativeNamesForHostname` on Kotlin) now matches the connecting host against the certificate's full set of presented names — Common Name *and* Subject Alternative Names — via a single shared matcher in the Rust core (`SslCertificateInfo.hostIsAllowListed`), instead of hand-rolled per-platform Common Name parsing. Kotlin extracted the Common Name with a bare `session.peerPrincipal.name.replace("CN=", "")`, which left any other subject RDNs (`O`, `L`, `C`, …) attached and so never matched the configured key: the exception silently did nothing for the ordinary multi-RDN certificate it exists to serve, and the opted-in host still failed with the certificate error. Swift keyed on the Common Name alone, so a SAN-only certificate (which omits the Common Name) could not be allow-listed at all. The matcher is case-insensitive and unit-tested in `wp_api` against multi-RDN and SAN-only certificates, and the native trust callbacks now skip the certificate parse entirely when no exception is configured.
...
- **Swift:** The certificate-name exception — `allowAlternativeNames(_:forCommonName:)`, formerly `allowSSL(altNames:forCommonName:)` — now re-evaluates the server trust with `SecTrustEvaluateWithError` — under a hostname-relaxed `SecPolicyCreateSSL(true, nil)` policy — before accepting the authentication challenge. Previously the executor's server-trust delegate returned `.useCredential` as soon as the leaf certificate's common name matched the allow-list, so chain signature, issuer, and validity-date checks were skipped entirely and a self-signed certificate copying a legitimate common name would be accepted for any allow-listed host. Chain validation is now enforced; only the hostname check the feature is meant to waive is relaxed, and a certificate that fails re-evaluation falls through to default handling. Consumers that deliberately relied on the previous accept-anything behavior can opt back into it per host with the new `disableCertificateValidation(forHost:)`.
