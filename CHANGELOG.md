...
- `redact_request_url_for_log`, `summarize_response_body_for_log` and `redact_response_text_for_log` reduce the fields of a failed request that can carry secrets — the request URL, the response body, and free text lifted out of that body — to a chosen level of detail. `WpRequestUrlLogDetail` offers `PathOnly`, `QueryKeysOnly` (keys kept, values replaced with `REDACTED`, except `rest_route`, which carries the endpoint on plain-permalink sites) and `Full`; `WpResponseBodyLogDetail` offers `Omitted`, `Summary` (byte count, shape, and a JSON object's top-level keys) and `Full`. Exported over UniFFI, so a Swift caller can use the same primitives; nothing in the Swift wrapper calls them yet.
- Kotlin: `RequestErrorLogPolicy` pairs a `WpRequestUrlLogDetail` with a `WpResponseBodyLogDetail` and decides how much of a failed request `toLogErrorString` writes down. A `RequestErrorLogger` carries its own policy, so a client whose URLs hold credentials can log less than the rest without changing what the others log: `RequestErrorLogger.withPolicy(RequestErrorLogPolicy(PATH_ONLY, OMITTED)) { message -> … }`. A logger written as a lambda keeps the default policy.
...
- The `hostname` on `RequestExecutionErrorReason`'s `HttpAuthenticationRequiredError`, `HttpAuthenticationRejectedError` and `HttpForbiddenError` is now the host (with its port, when the URL names one) rather than the entire request URL. These reasons are built for any 401 or 403 whose body isn't a `WpError` — a WAF block, an HTTP Basic auth prompt, a Cloudflare challenge — so the field carried the query string wherever the reason went, including a password-protected post's `password` on a 403 from `GET /wp/v2/posts/<id>`. It is rendered into the localized message these reasons produce (`The server at {$url} requires authentication.`), so that message now names the site rather than the whole request.
