...
- The `reqwest` request executor no longer carries the request URL into a `RequestExecutionErrorReason`'s error text. `reqwest::Error`'s `Display` ends with ` for url (…)`, so a transport failure put the URL, query string and all, into both any log line printing the reason and the localized message the reason produces. The executor drops it before stringifying; the URL is still reported separately, where it is redacted.
