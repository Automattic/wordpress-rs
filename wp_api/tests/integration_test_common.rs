...
pub fn read_test_credentials_from_file() -> TestCredentials {
    ...
    if lines.is_empty() {
        panic!("Invalid Test Credentials – unable to continue");
    }

    TestCredentials {
        site_url: std::env::var("API_URL").unwrap_or(lines[0].to_string()),
    ...
    Fut: Future<Output = ()>,
{
    ...
    let wp_content_path = std::env::var("WP_CONTENT_PATH").unwrap_or("/app/.wordpress/wp-content".to_string());
    ...
    uu_cp::copy(&[PathBuf::from(wp_content_path.clone() + "/plugins-backup")],  &PathBuf::from(wp_content_path.clone() + "/plugins"), &options)
