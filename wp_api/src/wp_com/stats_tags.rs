...
/// A group of tags with their combined view count.
#[derive(Debug, Clone, Serialize, Deserialize, uniffi::Record)]
pub struct StatsTagGroup {
    /// The tags in this group.
    pub tags: Vec<StatsTag>,
...
#[derive(Debug, Clone, Serialize, Deserialize, uniffi::Record)]
pub struct StatsTag {
    /// The type of the tag (e.g., "tag" or "category").
    #[serde(rename = "type")]
    pub tag_type: String,
    /// The name of the tag.
    pub name: String,
    /// The URL link to the tag page.
    pub link: String,
}
...
mod tests {
    ...
    fn test_stats_tags_response_deserialization() {
        let file = std::fs::File::open("tests/wpcom/stats_tags/tags-01.json")
    ...
    fn test_stats_tags_response_deserialization_empty() {
        let file = std::fs::File::open("tests/wpcom/stats_tags/tags-02-empty.json")
            .expect("Failed to open file");
        let response: StatsTagsResponse =
            serde_json::from_reader(file).expect("Unable to parse JSON");
