...
request_execution_failed = Échec de l'envoi HTTP
...
invalid_header_value_error = Valeur d'en-tête invalide : {$header_value}

http_auth_method_missing_nonce = Nonce manquant dans la méthode d'authentification HTTP
http_auth_method_missing_qop = QOP (Qualité de Protection) manquant dans la méthode d'authentification HTTP
http_auth_method_missing_algorithm = Algorithme manquant dans la méthode d'authentification HTTP
http_auth_method_missing_opaque = Valeur opaque manquante dans la méthode d'authentification HTTP
http_auth_method_unknown = Méthode d'authentification HTTP inconnue

uniffi_serialization_error_serde = Erreur de sérialisation : {$reason}

uuid_parse_error_invalid_uuid = Chaîne UUID invalide
