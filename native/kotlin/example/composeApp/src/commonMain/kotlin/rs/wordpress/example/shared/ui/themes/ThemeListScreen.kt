...
fun ThemeListScreen(
    ...
    viewModel: ThemeListViewModel = remember { ThemeListViewModel(apiClient) },
    onBackClicked: () -> Unit = {}
) {
    ...
    Scaffold(
        ...
        if (isLoading) {
            ...
            LazyColumn(
                ...
                items(themes) { theme ->
                    ListItem(
                        headlineContent = { Text(theme.name.rendered) },
                        supportingContent = { Text("v${theme.version}") },
                        overlineContent = { Text(theme.status.toString()) }
                    )
                }
            }
        }
