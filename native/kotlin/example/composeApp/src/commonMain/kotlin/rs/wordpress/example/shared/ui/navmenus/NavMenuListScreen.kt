...
@Composable
@Preview
fun NavMenuListScreen(
    apiClient: WpApiClient,
    viewModel: NavMenuListViewModel = remember { NavMenuListViewModel(apiClient) },
    onBackClicked: () -> Unit = {}
) {
    ...
    Scaffold(
        ...
        if (isLoading) {
            ...
            LazyColumn(
                ...
                items(navMenus) { navMenu ->
                    ListItem(
                        headlineContent = { Text(navMenu.name) },
                        supportingContent = { Text(navMenu.description) }
                    )
                }
            }
        }
